...
- fromService:
  - Allows you to use the built image from another service.
  - Requires adding a `fromService: [serviceId]` parameter in your code
  - This is only supported for container based services like `fargate` and `fargate-worker`
- fromRepository:
  - Allows you to use a pre-built image from a repository
  - Requires you to create an [image registry](/guides/advanced/deploying-own-image) and add the `containerImage` parameter.
  - This is only supported for container based services like `fargate` and `fargate-worker`
...
{/* need to indent these values */}
`registryId: string`

      - Example: `"registryId": "docker-hub-e13j2"`
      - This is the ID of the registry that you have created in the UI.

      `repostory: string`

      - Example: `"repository": "/something/whatever"`
