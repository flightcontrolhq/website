...
If you still prefer to setup a root domain, make sure that it accepts an ALIAS DNS Record, which is a custom one not supported by all providers.
We know Namecheap supports it, but any with that feature will work.

## Steps

To connect a custom domain, you'll need to follow these high-level steps:

1. Add the domain to your service configuration
...
### Add The Domain To Your Service
...
Once the domain information is saved and there is a deployment, the required DNS changes will be visible in the Flightcontrol dashboard, in the environment where your service is at.
