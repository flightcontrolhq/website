...
- This build type allows you to use a pre-built image from an (image registry)[link]. You must first create an image registry. Then, you specify the repository and the image tag to point to the image you want us to use. See (image registry) for more details on how to deploy your own image.

## From Service
- Currently, this is only supported using code.
